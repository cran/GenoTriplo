...
GenoAssign_pop_dif = function(resClustering,SampleName,NbClustMax,SeuilNoCall,SeuilSD,SeuilNbSD,Dataset,cr_marker=NULL,fld_marker=NULL,hetso_marker=NULL,verbose=FALSE){
  ...
  if (NbClustMax==3){ # on regarde le nombre de genotype maximum : ici diploide
    ...
    Mu_gp2 = mean(Mean_max_clust$Gp2)
    ...
    for (k in MarkerName[!MarkerName %in% c(list_max_clust,list_max_clust_false)]){ # mtn quon a les moyennes, on peut assigner le reste
      ...
      if (! is.character(resClustering[[k]])){ # Verification que le clustering a bien fonctionne pour ce marker
        ...
        if (n_clust_restant==1){ # si il reste plus quun groupe
          m1=resClustering[[k]]$means[val_clust[1]] # stockage de son centre
          ...
          resGenoAssign[k,which(resClustering[[k]]$df$partition==val_clust[1])]=-g1+4-1 # assignation de ce groupe
    ...
    Mu_gp1 = mean(Mean_max_clust$Gp1)
    Mu_gp2 = mean(Mean_max_clust$Gp2)
    Mu_gp3 = mean(Mean_max_clust$Gp3)
    ...
    for (k in MarkerName[!MarkerName %in% c(list_max_clust,list_max_clust_false)]){ # mtn quon a les moyennes, on peut assigner le reste
      ...
      if (n_clust_restant==1){
        ...
        D1=abs(Mu_tot-m1)
        g1=which(D1==min(D1))
        resGenoAssign[k,which(resClustering[[k]]$df$partition==val_clust[1])]=-g1+5-1
      } else if (n_clust_restant==2){
        m1=resClustering[[k]]$means[val_clust[1]]
        m2=resClustering[[k]]$means[val_clust[2]]
        D1=abs(Mu_tot-m1)
        ...
        resGenoAssign[k,which(resClustering[[k]]$df$partition==val_clust[1])]=-g1+5-1 # y=ax+b avec y=1=>x=3;y=2=>x=2;y=1=>x=1 puis -1 car valeur = 0,1,2 et non 1,2,3
        resGenoAssign[k,which(resClustering[[k]]$df$partition==val_clust[2])]=-g2+5-1
        ...
        resGenoAssign[k,which(resClustering[[k]]$df$partition==val_clust[1])]=-g1+5-1 # y=ax+b avec y=1=>x=3;y=2=>x=2;y=1=>x=1 puis -1 car valeur = 0,1,2 et non 1,2,3
        resGenoAssign[k,which(resClustering[[k]]$df$partition==val_clust[2])]=-g2+5-1
        resGenoAssign[k,which(resClustering[[k]]$df$partition==val_clust[3])]=-g3+5-1
      }
      # tmp = apply(resClustering[[k]]@proba,MARGIN=1,FUN=max)
      resGenoAssign[k,resClustering[[k]]$df$proba<SeuilNoCall]=-1
      resGenoAssign[k,is.na(resClustering[[k]]$df$partition)]=-1
